...
const MealSingle = ({meal}) => {
  ...
  const [Recipe, setRecipe] = useState({recipe_name:meal.title,recipe_country:meal.area,idmeal:meal.id,recipe_img:meal.thumbnail,recipe_tag:meal.tags,iduser:CurrentUser.iduser})
  // let ingredientList = meal.ingredients.join(", ")
 
  // let ingrediantMeasurement=meal.measures.join(", ")
  const [Ingredient, setIngredient] = useState({ingrediant_list:"",ingrediant_mmeasurement:"",ingrediant_instruction:meal.instructions,youtube_link:meal.youtube,idrecipe:0})
  console.log(meal);
  ...
  const saveIngrediant=async (data)=>{
    ...
    console.log(data)
    try {
      const res = await axios.post("http://localhost:8000/api/recipe/addingrediant", Ingredient)
      if(res.data==="Recipe has been created") alert("Recipe has been created")
    } catch (error) {
  ...
  const getRecipeId =async ()=>{
    try {
      const res = await axios.post("http://localhost:8000/api/recipe/getrecipeid", Recipe)
...
const Save = async ()=>{
  try {
    const res = await axios.post("http://localhost:8000/api/recipe/addrecipe", Recipe)
    console.log(res)
        ...
        <section className='sc-details bg-white'>
          <div className='details-head grid'>
            ...
            <div className='details-intro'>
              ...
              <div className='tags flex align-start flex-wrap'>
                <h6 className='fs-16'><button classname="btn" onClick={Save}>Save</button></h6>
               <a href={ meal?.youtube}>Watch Video</a>
              </div>
